...
class OAuth
{
    ...
    public function authenticate()
    {
        if ($this->access_token != null) {
            $query = "SELECT * FROM API.session WHERE access_token = '$this->access_token';";
    ...
    {
        if (
            $this->refresh_token != null and
            !$this->startsWith($this->refresh_token, "d.")
        ) {
            $query = "SELECT * FROM API.session WHERE refresh_token = '$this->refresh_token';";
            $result = mysqli_query($this->db, $query);
            if ($result) {
                $data = mysqli_fetch_assoc($result);
